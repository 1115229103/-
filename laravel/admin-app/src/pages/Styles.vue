...
onMounted(async () => {
  ...
  } catch { styles.value = []; loadError.value = '加载失败，请检查网络后重试'; }
  loading.value = false;
});
</script>
  ...
  <div>
    ...
    <table v-else class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="styles.length === 0">
          <td colspan="6" style="text-align:center;color:var(--text-muted);padding:32px">暂无风格预设</td>
        </tr>
      </tbody>
    </table>
  </div>
